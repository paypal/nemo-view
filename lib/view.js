...
var locreator = function (thees, context, loc, _loc, el) {
        ...
        context[loc + "Wait"] = (function (timeout, msg) {
            return thees.drivex.waitForElement(_loc, timeout || 5000, msg).then(function (find) {
                if (find === true) {
                    return thees.wd.promise.fulfilled();
                } else {
                    return thees.wd.promise.rejected(new Error('Element with locator ' + JSON.stringify(_loc) + ' was not found'));
                }
            }); //wait(_loc, timeout, msg);
        }).bind(thees);
        context[loc + "WaitVisible"] = (function (timeout, msg) {
            return thees.driver.wait(function () {
                return thees.drivex.visible(_loc).then(function (isVisible) {
                    return isVisible;
                });
            }, timeout || 5000, msg).then(function (find) {
                if (find === true) {
                    return thees.drivex.find(_loc);
                } else {
                    return this.wd.promise.rejected(new Error('Element with locator ' + _loc + ' was not visible even after waiting for ' + timeout));
                }
            });
        }).bind(thees);
