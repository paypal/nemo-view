...
class Locreator {
    ...
    addStarMethods(locatorId, locatorJSON, parentWebElement) {
        ...
        locatorObject[locatorId + 'ClearText'] = locatorObject[locatorId].clearText = () => drivex.find(locator(), parentWebElement).then(async (elem) => {  const value = await elem.getAttribute('value');
                                                                                                                                                        const backspaceInput = new Array((value || '').length).fill('\b').join('');
                                                                                                                                                        await webElement().sendKeys(backspaceInput); });
