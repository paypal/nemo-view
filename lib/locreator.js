...
var Locreator = function (nemo) {
  ...
  this.normify = this.normalize = function (locator) {
    if(!locator.locator || !locator.locator.trim()) {
      error('NO or EMPTY locator found, please fix it');
    }
    if(!locator.type || !locator.type.trim()) {
      error('Locator type is empty or blank, DEFAULTING to css');
      locator.type = 'css';
    } else if(!nemo.wd.By[locator.type]){
      error('NOT a valid locator type, DEFAULTING to css');
      locator.type = 'css';
    }
