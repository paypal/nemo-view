...
class Locreator {
    ...
    addStarMethods(locatorId, locatorJSON, parentWebElement) {
        ...
        locatorObject[locatorId + 'AssertText'] = locatorObject[locatorId].assertText = (value) => drivex.assertText(locator(), parentWebElement, value);

        locatorObject[locatorId + 'AssertValue'] = locatorObject[locatorId].assertValue = (value) => drivex.assertValue(locator(), parentWebElement, value);
        ...
        locatorObject[locatorId].is = (value) => drivex.is(locator(), parentWebElement, value);

        locatorObject[locatorId].waitFor = (value, timeout) => drivex.waitFor(locator(), timeout || 5000, value, `Wait for ${value} failed for locator [${locatorId}]`);

        locatorObject[locatorId].waitForText = (value, timeout) => drivex.waitForText(locator(), timeout || 5000, value, `Wait for Text ${JSON.stringify(value)} failed for locator [${locatorId}]`);

        locatorObject[locatorId].waitForAttribute = (value, timeout) => drivex.waitForAttribute(locator(), timeout || 5000, value, `Wait for Attribute ${JSON.stringify(value)} failed for locator [${locatorId}]`);

        locatorObject[locatorId].waitForCssValue = (value, timeout) => drivex.waitForCssValue(locator(), timeout || 5000, value, `Wait for CssValue ${JSON.stringify(value)} failed for locator [${locatorId}]`);


        locatorObject[locatorId + 'AssertSelectedText'] = locatorObject[locatorId].assertSelectedText = (value) => drivex.assertSelectedText(locator(), parentWebElement, value);
