...
async function waitForCondition(driver, wd, ele, timeout, expectedStatus){
    ...
    switch (expectedStatus) {
        ...
        case "notSelected": case "notChecked":  case "unchecked": case "unselected": 
            ...
            break;  
        default:
            let types = ["visible","enabled","editable", "disabled", "notEditable", "selected", "checked"]
            throw new TypeError(`expected type should be one of: ['${types.join("', '")}']`)
    }
}
...
//     let types = {
//         "visible": wd.until.elementIsVisible,
//         "enabled|editable": wd.until.elementIsEnabled,
//         "disabled|notEditable": wd.until.elementIsDisabled,
//         "selected|checked": wd.until.elementIsSelected,
//         "notSelected|notChecked|unchecked": wd.until.elementIsNotSelected,
//     }
//     let keys = Object.keys(types).flatMap(type => type.split('|'))
//     if(keys.includes(expectedStatus)){
//         Object.keys(types).forEach(type => {
//             if(type.split('|').includes(expectedStatus)){
//                 await driver.wait(types[type](ele), timeout)
//             }
//         })
//     }
//     else{
//         throw new TypeError(`expected type should be one of: ['${keys.join("', '")}']`)
//     }
// }
...
function doAssert(actual, expected, msg){
    ...
    switch (comparator) {
        ...
        case "noLessThan": case ">=":
            ...
            break;                  
        default:
            let types = ["equal","contains","match"]
            throw new TypeError(`expected type should be one of: ['${types.join("', '")}']`)
    }
}

async function comparator(actual, expected){
