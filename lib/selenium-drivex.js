...
module.exports = function drivex(driver, wd) {
    var methods = {
        ...
        waitFor: async function (locator, timeout, expected) {
            await driver.wait(wd.until.elementLocated(locator), timeout * 1000).then(
                function () {
                    log("waitForElement: found", locator);
                    return methods.find(locator);
                },
                ...
                try{
                    await waitForCondition(ele, timeout * 1000, expected)
                }
                catch(err){
                    throw new Error(err)
                }
...
async function waitForCondition(ele, timeout, expectedStatus){
